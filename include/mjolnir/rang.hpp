    ...
    namespace rang_implementation {
        ...
        inline bool supportsColor() noexcept {
            ...
            static const bool result = [] {
                char const *Terms[] = {"ansi",  "color",   "console", "cygwin",
        ...
        inline bool isMsysPty(int fd) noexcept {
            ...
            auto const ptrGetFileInformationByHandleEx =
            ...
            auto pNameInfo =
                    std::unique_ptr<MY_FILE_NAME_INFO>(new (std::nothrow
                    ) MY_FILE_NAME_INFO());
            if (!pNameInfo) {
                return false;
            }
...
#if defined(OS_LINUX) || defined(OS_MAC)
            if (osbuf == cout.rdbuf()) {
                static bool const cout_term = isatty(fileno(stdout)) != 0;
                return cout_term;
            } else if (osbuf == cerr.rdbuf() || osbuf == clog.rdbuf()) {
                static bool const cerr_term = isatty(fileno(stderr)) != 0;
                return cerr_term;
            }
#elif defined(OS_WIN)
            if (osbuf == cout.rdbuf()) {
                static bool const cout_term =
                        (_isatty(_fileno(stdout)) || isMsysPty(_fileno(stdout))
                        );
                return cout_term;
            } else if (osbuf == cerr.rdbuf() || osbuf == clog.rdbuf()) {
                static bool const cerr_term =
        ...
        enum class
                AttrColor : BYTE {// Color attributes for console screen buffer
                    black   = 0,
                    red     = 4,
                    green   = 2,
                    yellow  = 6,
        ...
        inline HANDLE getConsoleHandle(std::streambuf const *osbuf) noexcept {
        ...
        inline bool setWinTermAnsiColors(std::streambuf const *osbuf) noexcept {
        ...
        inline bool supportsAnsi(std::streambuf const *osbuf) noexcept {
            using std::cerr;
            using std::clog;
            using std::cout;
            if (osbuf == cout.rdbuf()) {
                static bool const cout_ansi =
                        (isMsysPty(_fileno(stdout)) ||
                         setWinTermAnsiColors(osbuf));
                return cout_ansi;
            } else if (osbuf == cerr.rdbuf() || osbuf == clog.rdbuf()) {
                static bool const cerr_ansi =
                        (isMsysPty(_fileno(stderr)) ||
                         setWinTermAnsiColors(osbuf));
                return cerr_ansi;
            }
            return false;
            ...
            static const SGR defaultSgr = []() -> SGR {
                ...
                if (GetConsoleScreenBufferInfo(
                    ...
                    GetConsoleScreenBufferInfo(
                            GetStdHandle(STD_ERROR_HANDLE), &info
                    )) {
        ...
        inline void setWinSGR(rang::fgB col, SGR &state) noexcept {
            state.fgColor = FOREGROUND_INTENSITY |
                            ansi2attr(static_cast<BYTE>(col) - 90);
        }
